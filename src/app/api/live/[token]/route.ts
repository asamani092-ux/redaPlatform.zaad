...
export async function GET(
  ...
  try {
    ...
    console.error("live metrics GET failed:", e);
    const msg = e instanceof Error ? e.message : "خطأ داخلي";
    const hint =
      msg.includes("volunteerTeam") || msg.includes("Volunteer")
        ? " — ربما تحتاج prisma migrate deploy (0015_volunteer_team)"
        : "";
    return NextResponse.json(
      { error: `تعذر تحميل العرض الحي${hint}` },
      { status: 500 },
    );
  }
}
